...
def safe_text(value, default=""):
    """
    Protège les textes utilisés dans les Paragraph ReportLab.
    """
    if value is None:
        return default
...
def get_image(path, width, height, fallback):
    """
    Charge une image si elle existe.
    Sinon affiche un texte de remplacement.
    """
...
def calculer_moyenne_generale_etudiant(
    etudiant,
    semestre,
):
    """
    ...
    1. Moyenne ECUE pondérée par coefficient
    2. Moyenne UE
    3. Moyenne générale = moyenne simple des UE
    """
...
def calculer_rang_etudiant(
    ...
    for camarade in camarades:
        ...
        moyenne = calculer_moyenne_generale_etudiant(
            camarade,
            semestre,
        )

        moyennes_autres.append(
            moyenne
        )

    effectif = (
        len(moyennes_autres) + 1
    )
...
# =========================================================
# BULLETIN GESTION ET DROIT
# =========================================================

def generer_bulletin_gestion_droit_pdf(
    etudiant,
    semestre,
    file_path,
):
    ...
    # IMPORTANT :
    # même taille que le premier bulletin
    logo = get_image(
        logo_path,
        3.2 * cm,
        3.2 * cm,
    ...
    if etudiant.niveau in (
        ...
        specialite = (
            ...
            if etudiant.filiere
            else "SCIENCES ECONOMIQUES & DE GESTION"
        )

    specialite = safe_text(
        specialite.upper()
    )
    ...
    stats = {
        ...
        "credits_ecue_total": 0.0,
        "credits_ecue_obtenus": 0.0,
    ...
    credits_ue_restants = round(
        credits_ue_total
        - credits_ue_acquis,
        2,
    )

    credits_ecue_restants = round(
        credits_ecue_total
        - credits_ecue_acquis,
        2,
    )
...
def generer_bulletin_gestion_pdf(
    etudiant,
    semestre,
    file_path,
):
...
def generer_bulletin_lmd_pdf(
    ...
    # =====================================================
        ...
        raise ValueError(
            f"Aucun générateur PDF pour la filière "
            f"{filiere}"
        )
