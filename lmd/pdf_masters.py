...
def generer_bulletin_masters_pdf(etudiant,semestre,file_path):
    ...
    semestres = MasterUE.objects.filter(
    programme=etudiant.programme
    ).values_list(
    "semestre",
    flat=True
    ...
    annee = etudiant.programme.annee_academique
    ...
    cadre_universite = Table([[
        Paragraph(f"""
             <b>DOMAINE :<br/>
             {etudiant.programme.get_niveau_display()}
             </b><br/><br/>
    ...
    cadre_etudiant = Table([
        ...
        [
         ...
         Paragraph( etudiant.get_sexe_display() if etudiant.sexe else "-",SMALL)],
        ...
        [Paragraph("<b>Niveau</b>", SMALL),
         Paragraph(etudiant.get_niveau_display() if etudiant.niveau else "-",SMALL)],
